/**
 * TEE Settlement Route
 * 
 * Handles payment settlement for x402-tee-facilitator scheme.
 * Verifies EIP-712 signatures, checks balances in TEE ledger,
 * and settles to sellers via OmnibusVault.
 */
...
const logger = createLogger("tee-settle");
...
export function createTEESettleRouter(
	...
	router.post("/", async (req, res) => {
		try {
			...
			logger.info(`Seller: ${intent.seller}, Amount: ${intent.amount}`);
			...
			// 7. Update ledger (deduct from buyer)
			try {
				ledger.recordSpend(
					intent.buyer,
					intent.seller,
